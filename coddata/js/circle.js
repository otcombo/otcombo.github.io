...
function updatePos() {
  ...
  let tx = map(mouseX, 0, width, width / -50, width / 50);
  let ty = map(mouseY, 0, height, height / -30, height / 30);

  targetX = tx + width / 1.8;
  targetY = ty + height / 4;

  let speedX = 5 * Math.cos((currentX / targetX) * PI);
  let speedY = 5 * Math.cos((currentY / targetY) * PI);
  ...
  // console.log("b : " + speedX.toFixed(1), currentX.toFixed(0), targetX.toFixed(1), tx);
